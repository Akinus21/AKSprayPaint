...
fn extract_wallpaper_theme(
    input: &RgbImage,
    target: &NoctaliaTheme,
) -> Result<MatugenTheme, String> {
    ...
    let all_extracted: Vec<Oklch<f32>> = extracted_colors
        .iter()
...
fn get_hex_from_scheme(
    ...
    for try_key in &["default", "dark", "light"] {
        let Some(inner) = entry.get(*try_key) else {
            continue;
        };
...
fn parse_hex(hex: &str) -> Result<[u8; 3], String> {
    ...
    let r = u8::from_str_radix(&hex[0..2], 16).map_err(|_| format!("invalid hex: {}", hex))?;
    let g = u8::from_str_radix(&hex[2..4], 16).map_err(|_| format!("invalid hex: {}", hex))?;
    let b = u8::from_str_radix(&hex[4..6], 16).map_err(|_| format!("invalid hex: {}", hex))?;
    Ok([r, g, b])
}

fn build_anchor_mappings(
    source: &MatugenTheme,
    target: &NoctaliaTheme,
    verbose: bool,
    ...
    let slots: Vec<(&str, [u8; 3], [u8; 3])> = vec![
        ...
        (
            ...
            target.surface_variant,
        ),
        (
            "on_surface_variant",
            source.on_surface_variant,
            target.on_surface_variant,
        ),
    ];

    let mappings: Vec<_> = slots
        .into_iter()
        .map(|(slot_name, src_rgb, target_rgb)| {
            ...
            if verbose {
                eprintln!(
                    ...
                    src_oklch.l,
                    src_oklch.chroma,
                    src_oklch.hue.into_positive_degrees()
                );
                eprintln!(
                    "Target: {} | L:{:.3} C:{:.3} H:{:.1}°",
                    slot_name,
                    target_oklch.l,
                    target_oklch.chroma,
                    target_oklch.hue.into_positive_degrees()
                );
                eprintln!("---");
            }
...
fn extract_quantette_palette(input: &RgbImage, num_colors: usize) -> Vec<Oklch<f32>> {
    let raw_pixels: Vec<palette::Srgb<u8>> = input
        .pixels()
